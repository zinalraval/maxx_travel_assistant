...
# Fallback: City to IATA code mapping
CITY_TO_IATA = {
...
amadeus_token_cache = {"access_token": None, "expires_at": None}

def get_amadeus_access_token():
    if amadeus_token_cache["access_token"] and datetime.utcnow() < amadeus_token_cache["expires_at"]:
        return amadeus_token_cache["access_token"]
    ...
    data = response.json()
    amadeus_token_cache["access_token"] = data["access_token"]
    amadeus_token_cache["expires_at"] = datetime.utcnow() + timedelta(seconds=data["expires_in"])
...
async def voice_webhook(request: Request):
    try:
        ...
        if not voice_text:
            return JSONResponse(content={"response_text": "I didn't catch that. Can you please repeat?"})
        ...
        if session["state"] == "start":
            response_text = (
                ...
                "You can say things like ‘Book a flight from London to New York on August 20’ "
                "or ‘Find a hotel in Dubai next weekend’. What would you like to do?"
            )
            ...
            if origin and destination and date_str:
                ...
                if origin_code and destination_code:
                    ...
                    if r.status_code == 200 and r.json().get("flights"):
                        ...
                        response_text = "Sorry, I couldn't find flights for that route and date."
                else:
                    response_text = "I couldn't identify one of the cities. Please try again."
                ...
                if city_code:
                    ...
                    response_text = "I couldn't identify the hotel city. Try again with a major city."

            else:
                response_text = (
                    "I didn’t understand your request fully. "
                    "Try saying 'Book a flight from Delhi to Dubai on August 20'."
                )
            ...
            if "yes" in voice_text.lower():
                ...
                response_text = "Okay, let me know if you'd like to search for another flight."
                session["state"] = "awaiting_input"
            ...
            if "yes" in voice_text.lower():
                ...
                response_text = "Okay, let me know if you'd like to search for another hotel."
                session["state"] = "awaiting_input"

        else:
            response_text = "I'm Maxx! Say something like 'Book a hotel in Paris next week.'"

        voice_webhook.sessions[user_id] = session
        return JSONResponse(content={"response_text": response_text})

    except Exception as e:
        return JSONResponse(status_code=500, content={"response_text": f"Internal error: {str(e)}"})
